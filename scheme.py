...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        new_expressions = Pair(formals, body)
        env.define(name, do_lambda_form(new_expressions, env))
        return name
        # END PROBLEM 9
    else:
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        return True
    sub = scheme_eval(expressions.first, env)
    if expressions.rest is nil and is_true_primitive(sub):
        return sub
    if is_false_primitive(sub):
        return False
